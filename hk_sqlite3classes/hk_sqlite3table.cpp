...
hk_sqlite3table::hk_sqlite3table(hk_sqlite3database* db,hk_presentation* p):hk_sqlite3datasource(db,p)
...
hk_datasource::enum_datasourcetypes hk_sqlite3table::type() const
{
    return ds_table;
}


bool hk_sqlite3table::driver_specific_name(const hk_string& )
{
    return true;
}


bool hk_sqlite3table::driver_specific_enable(void)
{
//  p_readonly=false;
    return hk_sqlite3datasource::driver_specific_enable();

}


bool hk_sqlite3table::driver_specific_create_table_now(void)
{
#ifdef HK_DEBUG
    hkdebug("hk_sqlite3table::driver_specific_create_table_now");
#endif
...
hk_string hk_sqlite3table::getprimarystring(bool alter)
{
...
hk_string hk_sqlite3table::internal_new_fields_arguments(bool alter)
{
#ifdef HK_DEBUG
    hkdebug("hk_sqlite3table::internal_new_fields_arguments");
#endif
    hk_string csql;
    hk_string line;
    hk_string fields;
...
hk_string hk_sqlite3table::field2string(hk_column::enum_columntype f,const hk_string& m)
{
#ifdef HK_DEBUG
    hkdebug("hk_sqlite3table::field2string");
#endif
    hk_string fields;
    switch (f)
    {
...
bool hk_sqlite3table::driver_specific_alter_table_now(void)
{
#ifdef HK_DEBUG
    hkdebug("hk_sqlite3table::driver_specific_alter_table_now");
#endif
    ...
    if (r.size()>0)
    {
    error=hk_translate("hk_sqlite3table::  Driver does not support the deletion of columns!\n");
    ...
    asql+=r;
    cerr <<"ALTER SQL="<<asql<<endl;

    hk_actionquery* query= p_database->new_actionquery();
    if (!query) return false;

    query->set_sql(asql.c_str(),asql.size());
    bool result=query->execute();
    if (result) cerr<<"tabelle ge�dert"; else cerr<<"fehler";cerr <<endl;
    delete query;
    return result;
}


hk_string  hk_sqlite3table::internal_delete_fields_arguments(void)
{
#ifdef HK_DEBUG
    hkdebug("hk_sqlite3table::internal_delete_fields_arguments");
#endif
cerr <<"internal_delete_fields_arguments"<<endl;
    ...
    while (it!=p_deletecolumns.end())
    {
        ...
        it++;
    } cerr <<"internal_delete_fields_arguments return:#"<<result<<"#"<<endl;
    return result;
}


hk_string  hk_sqlite3table::internal_alter_fields_arguments(void)
{
#ifdef HK_DEBUG
    hkdebug("hk_sqlite3table::internal_alter_fields_arguments");
#endif
return "";
}


bool hk_sqlite3table::is_alteredfield(const hk_string f)
{
#ifdef HK_DEBUG
    hkdebug("hk_sqlite3table::is_alteredfield");
#endif
...
bool hk_sqlite3table::is_deletedfield(const hk_string f)
{
#ifdef HK_DEBUG
    hkdebug("hk_sqlite3table::is_deletedfield");
#endif
    list<hk_string>::iterator it=p_deletecolumns.begin();
    while (it!=p_deletecolumns.end())
    {
...
list<hk_datasource::indexclass>* hk_sqlite3table::driver_specific_indices(void)
{
    ...
    for(unsigned long i=0;i<max;++i)
    {
     ...
     {
     //cerr <<"sql.asstring "<<startpos<<endl;
...
list<hk_datasource::indexclass>::iterator hk_sqlite3table::findindex(const hk_string& i)
{
...
bool    hk_sqlite3table::driver_specific_drop_index(const hk_string& i)
{
    hk_string s;
        s+=" DROP INDEX ";
...
bool    hk_sqlite3table::driver_specific_create_index(const hk_string& i,bool unique, list<hk_string>& fields)
{
    hk_string s="CREATE ";
    if (unique)s+="UNIQUE ";
    s+="INDEX ";
...
void hk_sqlite3table::parse_indices(list<hk_string>& tokens, const hk_string & indiceslist)
{
