...
typedef struct {
	...
	int            object_type;
	uint32_t  table_pg; /* misnomer since object may not be a table */
	uint32_t  kkd_pg;
	uint32_t   kkd_rowid;
	int32_t		num_props;
	GArray		*props;
...
/* file.c */
extern ssize_t mdb_read_pg(MdbHandle *mdb, uint32_t pg);
extern ssize_t mdb_read_alt_pg(MdbHandle *mdb, uint32_t pg);
extern unsigned char mdb_get_byte(void *buf, int offset);
extern int16_t    mdb_get_int16(void *buf, int offset);
extern int32_t   mdb_get_int32(void *buf, int offset);
extern int32_t   mdb_get_int32_msb(void *buf, int offset);
...
extern int16_t    mdb_pg_get_int16(MdbHandle *mdb, int offset);
extern int32_t   mdb_pg_get_int32(MdbHandle *mdb, int offset);
